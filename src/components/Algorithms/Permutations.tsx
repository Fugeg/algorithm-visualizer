    ...
    <div className="max-w-6xl mx-auto p-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="space-y-6">
          ...
          <CodeSyncPanel
            title={PERMUTATIONS_PSEUDOCODE.title}
            codeLines={PERMUTATIONS_PSEUDOCODE.lines}
            highlightLine={
              playbackState.currentStepIndex >= 0 && playbackState.steps[playbackState.currentStepIndex]
                ? playbackState.steps[playbackState.currentStepIndex].type === 'try' ? 9
                  : playbackState.steps[playbackState.currentStepIndex].type === 'backtrack' ? 12
                  : playbackState.steps[playbackState.currentStepIndex].type === 'solution' ? 4
                  : -1
                : -1
            }
          />
              ...
              playbackState.currentStepIndex >= 0 && playbackState.steps[playbackState.currentStepIndex]
                ? (() => {
                    ...
                    if (stepState.current) {
                      vars['当前排列'] = `[${stepState.current.join(', ')}]`;
                    }
                    if (stepState.used) {
                      vars['可用位置'] = stepState.used.filter((u: boolean) => !u).length;
                    }
                    if (stepState.trying !== undefined) {
                      vars['尝试索引'] = stepState.trying;
                    }
                    if (stepState.backtrack !== undefined) {
                      vars['回溯索引'] = stepState.backtrack;
                    }
                    return vars;
                  })()
                : {}
